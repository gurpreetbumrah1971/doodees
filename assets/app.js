...
function upiProviderButtons() {
  return `
    <div class="upi-qr-card">
      ...
      <img class="upi-qr-image" src="${projectAssetUrl('/assets/upi-qr.jpg')}" alt="Doodee's UPI payment QR code" width="180" height="180">
      <a class="btn secondary upi-qr-download" href="${projectAssetUrl('/assets/upi-qr.jpg')}" download="doodees-upi-qr.jpg">Download QR code</a>
    </div>
  `;
}
...
// Values are trusted, hand-authored markup (not user input), so they render
// as-is rather than being escaped - this lets a note include a link.
const CATEGORY_NOTES = new Map([
  ['Burgers', 'Complimentary french fries with burger'],
  ['Pies & Tarts', 'For whole pie (1Kg / 500g) pies contact Doodee\'s on WhatsApp - <a href="https://wa.me/919819068372" target="_blank" rel="noopener">9819068372</a>'],
]);
function categoryNoteHtml(category) {
  const note = CATEGORY_NOTES.get(category);
  return note ? `<p class="menu-category-note">${note}</p>` : '';
}
...
function renderDatabaseMenu(items) {
  ...
  document.querySelectorAll('[data-menu-tabs]').forEach((tabGroup) => {
    ...
    groups.forEach(([category, categoryItems], index) => {
      ...
      panel.innerHTML = `${categoryNoteHtml(category)}<div class="menu-grid">${categoryItems.map(menuCardHtml).join('')}</div>`;
  ...
  document.querySelectorAll('.menu-accordion-layout').forEach((layout) => {
    ...
    groups.forEach(([category, categoryItems], index) => {
      ...
      details.innerHTML = `<summary>${escapeHtml(category)}</summary>${categoryNoteHtml(category)}<div class="menu-grid">${categoryItems.map(menuCardHtml).join('')}</div>`;
...
function initHeroVideoCarousel() {
  document.querySelectorAll('[data-hero-carousel]').forEach((carousel) => {
    const videos = [...carousel.querySelectorAll('.hero-video-el')];
    if (videos.length < 2) return;

    let index = Math.max(0, videos.findIndex((video) => video.classList.contains('is-active')));
    videos.forEach((video) => { video.loop = false; });

    const dotsHost = carousel.parentElement && carousel.parentElement.querySelector('[data-hero-dots]');
    const dots = dotsHost
      ? videos.map((_, dotIndex) => {
        const dot = document.createElement('button');
        dot.type = 'button';
        dot.className = 'hero-video-dot' + (dotIndex === index ? ' is-active' : '');
    ...
    const goTo = (nextIndex) => {
      ...
      next.currentTime = 0;
      next.play().catch(() => {});
      // Let the crossfade finish before pausing the outgoing clip in the background.
      window.setTimeout(() => current.pause(), 650);
    ...
    videos.forEach((video) => video.addEventListener('ended', () => goTo((index + 1) % videos.length)));
  });
}

function initMenuGridReveal() {
  const cards = [...document.querySelectorAll('.menu-accordion .menu-card:not(.is-visible)')];
  ...
  return `
    <article class="card menu-card" data-menu-item data-name="${escapeHtml(`${item.name} ${item.description}`.toLowerCase())}" data-veg="${item.isVeg ? 'veg' : 'nonveg'}" data-bestseller="${item.bestseller ? '1' : '0'}">
      <div class="image-wrap">
        ...
        <span class="badge ${item.isVeg ? 'veg' : 'nonveg'}" role="img" aria-label="${item.isVeg ? 'Veg' : 'Non-Veg'}" title="${item.isVeg ? 'Veg' : 'Non-Veg'}"></span>
...
function statusMessage(status) {
  ...
  if (status.statusLabel === 'DELIVERED') return 'Delivered. Thank you for ordering from Doodee\'s.';
...
function showStaticOrderThankYou({ order, total, amount, paymentMethod, whatsappUrl, session }) {
  ...
  const shareText = `Order from Doodee's and get 5% off your first order with my referral code\n\n\`\`\`${referralCode}\`\`\`\n\nVisit - ${shareUrl}`;
  section.innerHTML = `
  ...
  if (form && form.hasAttribute('data-static-checkout')) {
    ...
    try {
      ...
      const whatsappUrl = `https://wa.me/${normalizedNumber}?text=${encodeURIComponent([
        ...
        `Total: ${total}`,
        '',
        "Thank you for ordering from Doodee's.",
      ].join('\n'))}`;
...
async function initAddToOrderMode() {
  ...
  if (summaryCard && itemsTarget && !summaryCard.querySelector('.add-order-baseline-note')) {
    ...
    note.textContent = `Already ordered: ${itemCount} item${itemCount === 1 ? '' : 's'} Â· ${money(order.totalAmount)} food subtotal so far. Totals below include this order.`;
...
function enhanceSelect(select) {
  ...
  const uid = 'custom-select-' + (customSelectIdCounter += 1);
  ...
  function highlight(index) {
    ...
    highlightedIndex = Math.max(0, Math.min(index, optionEls.length - 1));
    optionEls.forEach((el, i) => el.classList.toggle('is-highlighted', i === highlightedIndex));
    trigger.setAttribute('aria-activedescendant', optionEls[highlightedIndex].id);
  ...
  function renderList() {
    list.innerHTML = '';
    optionEls = Array.from(select.options).map((opt, index) => {
      const li = document.createElement('li');
      li.className = 'custom-select-option';
      li.id = uid + '-option-' + index;
